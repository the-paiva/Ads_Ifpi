...
from utils.io_utils import pedir_int_min_max, pedir_int_min, pedir_float_min_max, pedir_float, pedir_float_min, pedir_int, pedir_string
from utils.string_utils import limpar_tela, enter_para_limpar_tela
from utils.math_utils import calcular_media_aritmetica, truncar_casas_decimais
from random import uniform
from os import path
from utils import vetor_utils
...
#Conjunto de mensagens de erro padronizadas
...
menu_principal = lambda: print(MENSAGEM_DO_MENU_PRINCIPAL)
...
def menu_de_inicializacao_de_vetor(CASAS_DECIMAIS):
    ...
    input_de_limite_inferior, input_de_limite_superior = atribuir_input_de_limite_inferior_e_superior(digito_de_geracao)
    ...
    if digito_de_geracao == 1:
        vetor = vetor_utils.inicializar_vetor_aleatoriamente_in_range(quant_elementos, limite_inferior, limite_superior)
        ...
        vetor = vetor_utils.inicializar_vetor_com_arquivo(nome_do_arquivo)
...
#Agregada de menu_de_inicializacao_de_vetor - Serve para atribuir as mensagens de input do limite inferior
#e do limite superior de números a duas variáveis, dependendo da opção digitada pelo usuário
def atribuir_input_de_limite_inferior_e_superior(digito_de_geracao):
    if digito_de_geracao == 1 or digito_de_geracao == 2:
    ...
    return input_de_limite_inferior, input_de_limite_superior
...
def menu_de_relatorio_de_valores(vetor):
    mensagem = (lambda: vetor_utils.escrever_vetor_e_posicoes(vetor) if len(vetor) > 0 
    else escrever_mensagem_de_erro(MENSAGEM_DE_VETOR_VAZIO))
...
def menu_de_reset_de_valores(vetor, CASAS_DECIMAIS):
    ...
    if len(vetor) > 0:
        ...
        vetor = vetor_utils.resetar_valores(vetor, valor_padrao)
        vetor = vetor_utils.truncar_casas_decimais_de_vetor(vetor, CASAS_DECIMAIS)
    else:
...
def menu_de_quantidade_de_elementos(vetor):
    mensagem = (lambda: print(f'Quantidade de elementos: {len(vetor)}') if len(vetor) > 0 
    else escrever_mensagem_de_erro(MENSAGEM_DE_VETOR_VAZIO))
...
def menu_de_somatorio(vetor):
    somatorio = vetor_utils.calcular_somatorio(vetor, 0)
    mensagem = (lambda: print(f'Somatório dos valores: {somatorio:.2f}') if len(vetor) > 0
    else escrever_mensagem_de_erro(MENSAGEM_DE_VETOR_VAZIO))
...
def menu_de_media_aritmetica(vetor):
    ...
    if len(vetor) > 0:
        VALOR_INICIAL_DE_SOMATORIO = 0
        somatorio = vetor_utils.calcular_somatorio(vetor, VALOR_INICIAL_DE_SOMATORIO)
...
def menu_de_numeros_positivos(vetor):
    ...
    if len(vetor) > 0:
        ...
        if len(vetor_de_positivos) > 0:
            vetor_utils.escrever_vetor_e_posicoes(vetor_de_positivos)
...
def menu_de_numeros_negativos(vetor):
    ...
    if len(vetor) > 0:
        ...
        if len(vetor_de_negativos) > 0:
            vetor_utils.escrever_vetor_e_posicoes(vetor_de_negativos)
...
escrever_mensagem_de_erro = lambda mensagem_de_erro: print(mensagem_de_erro)


#Sai do programa, salvando automaticamente os valores do vetor em um arquivo
sair_do_programa = lambda vetor: vetor_utils.salvar_vetor_em_txt(vetor, 'automatico') if len(vetor) > 0 else 0
